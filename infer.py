...
def main():
    ...
    if not os.path.exists(output_dir_color):
    ...
    with torch.no_grad():
        for i in tqdm(range(len(test_images))):
            ...
            with autocast_ctx:
                ...
                task_emb = (
                    torch.tensor([1, 0]).float().unsqueeze(0).repeat(1, 1).to(device)
                )
                task_emb = torch.cat(
                ...
                output_color.save(
                    os.path.join(output_dir_color, f"{save_file_name}.png")
                )
                np.save(
                    os.path.join(output_dir_npy, f"{save_file_name}.npy"), output_npy
                )
